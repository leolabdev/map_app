...
export default class SettingsUtil {
    ...
    async #addCityCenters(force=false) {
        if(!force){
            const cityCentersFound = await this.#countAll('Area', 'areaName');
            if(cityCentersFound > 0)
                return true;
        }

        const cityCenters = await this.#convertCityCentersFileToGeoJson(this.#cityCentersFileLocation);
        if(!cityCenters){
            console.error(`No ${this.#cityCentersFileLocation} file found or it is in wrong format`);
            return false;
        }

        await this.#areaDAO.deleteAllCityCenters();
        ...
        const centersCreated = await this.#countAll('Area', 'areaName');
        return centersCreated === centersCount;
    }

    async #addTMSDataToDB(force=false, tmsAreaSizeM=15) {
        if(!force){
            const tmsCount = await this.#countAll('TMS', 'stationId');
            if(tmsCount > 0)
                return true;
        }
    
        await this.#tmsDAO.deleteAll();
    
        const tmsData = await this.#getTMSData(tmsAreaSizeM);
        if(!tmsData)
            return false;
    
        await this.#tmsDAO.createMultiple(tmsData);
        const tmsCount = await this.#countAll('TMS', 'stationId');
        return tmsCount === tmsData.length;
    }

    async #getTMSData(tmsAreaSizeM=15){
        ...
        try {
            ...
            for (const station of tmsData.features) {
                ...
                const polygonCoordinates = JSON.stringify(this.#generateSquareFromPoint({lon, lat}, tmsAreaSizeM));
    ...
    async #convertCityCentersFileToGeoJson(filePath) {
        try {
    ...
    #generateSquareFromPoint(center, sizeM) {
...
async function getSlowTMSIds(maxAcceptableValue = 90) {
