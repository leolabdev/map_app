process.env.DATABASE = 'mapApp_' + process.env.JEST_WORKER_ID;
...
module.exports = async function createDB() {
    ...
    await connection.query(dbInitScript, (error, results) => {
        if(error)
            return console.error(`Problems with ${process.env.DATABASE} DB`);
    });
